...
def pay(request):
    ...
    receiver_name=payee.first_name
...
def pay_a_friend(request):
    ...
    f=name.first_name
    match=check_password(passwd,password)
    ...
    if match:
        if balance>amount:
            ...
            profile.save()
            message1="TRANSACTION SUCCESFULL"
            time=timezone.now()
            ...
            message1="INSUFFICIENT ACCOUNT BALANCE"
    else:
        message1="invalid password"
    context={'amount':amount,'receiver':payee,'receiver_profile':pay1, 'message':message1}
